...
// ── #1147 item 5: the in-flight frequency lock has to cover cmd 17 too ──

TEST(LoraHopChangeAllowed, EnableRefusedOnceLockedForFlight) {
    // cmd 10 (retune) and the rendezvous cycle both stop at the flight lock;
    // cmd 17 did not, so a hop-enable moved an AIRBORNE rocket off its
    // operating channel onto a per-packet schedule 1.5 s later.
    EXPECT_FALSE(loraHopChangeAllowed(/*want_disabled=*/false, /*locked=*/true));
    EXPECT_TRUE(loraHopChangeAllowed(/*want_disabled=*/false, /*locked=*/false));
}

TEST(LoraHopChangeAllowed, DisableAllowedEvenLocked) {
    // The same asymmetry as the mute rule, for the same reason.  A disable
    // pins the rocket to the channel it is already transmitting on -- it can
    // never move it somewhere the base station is not listening -- and
    // refusing it would strand a rocket that took off hopping.
    EXPECT_TRUE(loraHopChangeAllowed(/*want_disabled=*/true, /*locked=*/true));
    EXPECT_TRUE(loraHopChangeAllowed(/*want_disabled=*/true, /*locked=*/false));
}

TEST(LoraHopChangeAllowed, OnlyTheEnableUnderLockIsRefused) {
    // Exhaustive over the 2x2: exactly one cell is a refusal.  Stated as a
    // whole table because the failure being fixed was a MISSING cell, not a
    // wrong one.
    int refusals = 0;
    for (int wd = 0; wd < 2; ++wd)
        for (int lk = 0; lk < 2; ++lk)
            if (!loraHopChangeAllowed(wd != 0, lk != 0)) refusals++;
    EXPECT_EQ(refusals, 1);
}

TEST(LoraHopChangeAllowed, AgreesWithTheFreqLockLatchItGuards) {
    // The lock this consults is computeFreqLockForFlight's latch, so walk a
    // realistic flight and confirm the enable is refused exactly across the
    // locked span -- INFLIGHT onwards, until LANDED or READY clears it.
    const RocketState flight[] = {INITIALIZATION, READY, PRELAUNCH, INFLIGHT,
                                  INFLIGHT, LANDED};
    const bool want_enable_allowed[] = {true, true, true, false, false, true};
    bool locked = false;
    for (size_t i = 0; i < sizeof(flight) / sizeof(flight[0]); ++i)
    {
        locked = computeFreqLockForFlight(locked, flight[i]);
        EXPECT_EQ(loraHopChangeAllowed(false, locked), want_enable_allowed[i])
            << "at step " << i << " state " << (unsigned)flight[i];
        // A disable is allowed at every step, locked or not.
        EXPECT_TRUE(loraHopChangeAllowed(true, locked)) << "at step " << i;
    }
}

TEST(LoraHopChangeAllowed, PrelaunchStillAllowsAnEnable) {
    // The lock latches on INFLIGHT, not on PRELAUNCH -- configuring the link
    // mode on the pad is the normal workflow and must keep working.
    bool locked = false;
    locked = computeFreqLockForFlight(locked, PRELAUNCH);
    ASSERT_FALSE(locked);
