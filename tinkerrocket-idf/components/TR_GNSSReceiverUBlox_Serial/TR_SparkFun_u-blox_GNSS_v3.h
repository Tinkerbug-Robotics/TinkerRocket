/* Modified to remove I2C for compatability with low lever
   esp32 I2C drivers.
   Further modified to use ESP-IDF native UART driver (no Arduino dependency).
...
public:
  ...
  //
  // The caller must have already installed the UART driver and configured
  // baud/pins before calling begin().
  //
  //  Parameter    Description
  //  ---------    ----------------------------
  //  uartPort     ESP-IDF UART port number (e.g. UART_NUM_1)
  //  maxWait      Maximum time (ms) to wait for a response
  //  assumeSuccess  If true, assume the module is present even without full handshake
  //  retval       true on success, false on startup failure
  //
  bool begin(uart_port_t uartPort, uint16_t maxWait = kUBLOXGNSSDefaultMaxWait, bool assumeSuccess = false)
  {
    // Setup Serial object and pass into the superclass
    setCommunicationBus(_serialBus);

    // Initialize the Serial bus class with the ESP-IDF UART port
    _serialBus.init(uartPort);
