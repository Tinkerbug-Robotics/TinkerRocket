...
| Staleness: how it is SIGNALLED | **The two platforms mark it differently, and that is settled (2026-09-13), not drift.** Each marks staleness exactly once, so neither silently renders stale data as live — they just pick a different instrument. iOS fades the whole dashboard to 0.5 (`staleOpacity`) and shows its banner **only on base-station links**; Android shows the red banner on **every** link type and fades nothing but the health card (`ReadinessHold.HELD_OPACITY`, #1039). Neither is being converged onto the other: giving Android the global fade would change the appearance of every card on the busiest screen for no information the banner is not already carrying. Recorded because it was re-discovered as a suspected bug during the #624 parity walk and the row below only ever described the banner half |
| Staleness: the banner | worsen-only overlay; **red** banner wording "STALE DATA — link degraded"; never silently show stale values as live. Red is a deliberate divergence from iOS, which renders this orange — reaffirmed 2026-08-12 as a launch-safety signal that should shout. SYNCING is **not** part of it: it is a normal startup state, rendered blue, and used to share the same red as a dead link |
...
| Sensor health | one dot per subsystem, green/**orange**/red/gray = OK/DEGRADED/BAD/NA, **wrapped** into balanced lines of at most six — never a horizontal scroll (iOS 2026-08-22, Android 2026-09-09 #1070: the row count is six core sensors + Storage + one per configured pyro channel, so a two-pyro flight makes nine dots and the trailing ones sat off-screen with nothing to say they existed — on the row that explains a "Do not fly" verdict) |
