...
**2026-08-15 post-fab amendment — both halves of this closure were reversed after quantitative review (PR #792, schematic+board now V10; applies identically to rocket-computer-mini, whose battery input was bit-for-bit this circuit).**

**CR2 is deleted, not relocated.** The ESD-clamp rationale did not survive numbers: CR2 is not a rated ESD part (no TLP, no IEC 61000-4-2, no specified clamping voltage), and the strike it was kept for — negative HBM on an unplugged pigtail — deposits 4–16 µJ (2–8 kV) into a drain rated I_DM −240 A, three orders of magnitude below damage. As a rectifier it is inert for every *positive* excursion until 30 V, above every rating on the node. The one case where it acts is a reversed pack: it forward-conducts **35–62 A through a 1 A avg / 5 A surge SOD-323** and fails **short** across the unfused pack — the exact hazard the strikethrough under "Other" below believed closed. Its original role (power-eco.md CHANGE 2: eFuse fast-trip negative clamp at `VBAT_CON`) had already been destroyed by this same relocation, which moved it to the wrong node. The board-level ESD posture this leaves is no looser than what was already accepted: J2's externally exposed pyro screw terminals have never had a clamp.

**Q11 is now AONR21321** (AOS, −30 V, **±25 V V_GS**, DFN3×3-EP). The hard-tied gate puts V_GS = −V_pack on the CSD25404Q3 continuously (70% of its ±12 V abs max), and the hot-plug LC ring — pigtail inductance against C41/C42 — peaks 13.4–16.0 V across every analysed corner: **the ±12 V abs max was exceeded on every pack connection**, with cumulative gate-oxide stress, and a 3S misconnect exceeded it on DC. The gate divider this file's M-9 note contemplates was made unnecessary by the part change instead — no TI P-channel in any package exceeds ±12 V, so the fix is a vendor change. The gate stays hard-tied with zero support parts. ⚠️ New footprint `DFN-8-1EP_3x3mm_P0.65mm_EP1.55x2.4mm`: **tab/EP = DRAIN** (`VBAT_J8`), the *mirror* of the old clip (= source) — a mirrored placement reverses the body diode and silently defeats reverse protection; placement review is safety-critical. Also fixed in passing: **Q11 pin 2 had been `unconnected-(Q11-DRAIN-Pad2)` in the schematic netlist since V9** — the drain wire crosses the pin mid-segment with no junction, masked on the board by the copper pour connecting all three drain pads; a junction now sits there.

**V9 as-built boards** (fabbed, bare, one-off test builds): populate the original CSD25404Q3, **leave CR2's land empty**, 2S packs only, short twisted pigtails, polarity check at connection. Do not solder-bridge Q11 D–S — that re-exposes the INA230/eFuse IN pins (−0.3 V abs max) to a reversed pack.
...
**Other:** pyro continuity LEDs run through 100 k (R9/R11/R13/R19) — tens of µA, likely invisible in daylight; the TPS62152 inductor is at the datasheet minimum with 1.89 A saturation against a 2.2 A current limit; ~~CR2 is a 1 A Schottky across an unfused 2S pack~~ *(closure corrected 2026-08-15: the 2026-08-09 note closed this claiming CR2 moved "behind" Q11, but the netlist has it **upstream**, pack-side — so on V9 as-built the hazard was real: a reversed pack forward-conducts 35–62 A through the 1 A part, which fails short. Genuinely closed by deleting CR2 outright — PR #792, V10, and see the M-9 amendment above; on fabbed V9 boards, closed by leaving CR2 unpopulated)*; no series R/C provision on the P4 USB pair although the S3 leg has 22 Ω; GPIO0 has no external pull-up; USB ESD array sits on an ~8 mm stub.
